...
class Test_stock_fifo_update_cost(TransactionCase):
        ...
        self.delivery = self.pick_order.create(
            {
                ...
                "move_line_ids_without_package": [
                    (
                        ...
                        {
                            "company_id": 1,
                            "product_uom_id": self.kitproduct.uom_id.id,
                            "product_id": self.kitproduct.id,
                            "qty_done": 1.0,
                            "location_dest_id": self.customer_location.id,
                            "location_id": self.stock_location.id,
    ...
    def test_01_1bom_2products(self):
        # Create a customer picking
        self.assertEqual(self.kitproduct.standard_price, 0)
        self.delivery.button_validate()
        self.assertEqual(self.kitproduct.standard_price, 9)
